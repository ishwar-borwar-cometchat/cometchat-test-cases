...
def classify_test_case(scenario_text):
    """Classify a test case into a testing type based on its scenario text."""
...
for filepath, prefix in files:
    ...
    # Read ALL data rows from ALL sheets and classify
    ...
    for sheet_name in wb.sheetnames:
        ...
        for row in ws.iter_rows(min_row=2, max_row=ws.max_row, values_only=True):
            ...
            classified[test_type].append(list(row))
    
    # Create new workbook with separate sheets
    ...
    for sheet_type in SHEET_ORDER:
        ...
        total += len(rows)
        sheet_summary.append(sheet_name + ': ' + str(len(rows)))
    
    new_wb.save(full_path)
    print(prefix + ' - ' + str(total) + ' test cases -> ' + ', '.join(sheet_summary))

print('\nAll files split into sheets by testing type!')
